...
struct GrammarMeMenuView: View {
    ...
    private var home: some View {
        Group {
            ...
            Text("Select text, then choose Services → Format with GrammarMe from the context menu.")
                .font(.caption).foregroundStyle(.secondary).fixedSize(horizontal: false, vertical: true)
            Button { formatClipboard() } label: {
                if isFormatting { ProgressView().controlSize(.small) }
                else { Label("Format with GrammarMe", systemImage: "wand.and.sparkles") }
            }
            .buttonStyle(.borderedProminent).disabled(apiKey.isEmpty || isFormatting)
